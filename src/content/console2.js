...
var gModes = { // set to defaults
	...
	"Lang": { "JS": true, "CSS": true, "XML": true },
	"Ctx": { "Chrome": true, "Content": true , "Resource": true }
};
...
function searchFilterbox()
{
	...
	var uss = Components.classes["@mozilla.org/network/io-service;1"]
			...
			.newURI("chrome://econsole2/content/textbox191.css", null, null);
...
/*const*/ var gBooleanPrefObserver = {
	mPrefBranch: Cc["@mozilla.org/preferences-service;1"].getService(Ci.nsIPrefBranch),
...
var gBlacklist = ("@mozilla.org/permissionmanager;1" in Cc)?{
	...
	mIOService: Cc["@mozilla.org/network/io-service;1"].getService(Ci.nsIIOService),
	mBlocking: {},
    mBlocklistPref: "extensions.errorconsole2.blocklist",
...
function openDomainlist()
{
	...
	if (gBlacklist.mPermissionManager)
	{
		openDialog("chrome://econsole2/content/domainlist.xul",
			"_blank",
			"chrome,dialog,resizable,centerscreen");
	}
	else
	{
		openDialog("chrome://econsole2/content/blocklist.xul",
			"_blank",
			"chrome,dialog,resizable,centerscreen",
			gBlacklist);
	}
...
function onEvalTypein()
{
	...
	if (/\S/.test(code))
	{
		...
		gEvaluator.contentWindow.location = "chrome://econsole2/content/blank.html"; // reset the iframe
